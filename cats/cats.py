...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        ...
        i = [diff_function(user_word, x, limit) for x in valid_words]
        min_i, min_w = min( zip(i,valid_words) , key= lambda item: item[0])
        if min_i > limit:
            return user_word
        else:
            return min_w
...
def pawssible_patches(start, goal, limit):
    ...
    if limit < 0:
        ...
        # BEGIN
        return min(pawssible_patches(start[1:], goal[1:], limit), limit+1)
        # END
...
def time_per_word(times_per_player, words):
    ...
    """
    # BEGIN PROBLEM 9
    time = [ [x[i+1] - x[i] for i in range(len(x)-1)] for x in times_per_player]
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    re = [[] for i in player_indices]
    for w in word_indices:
        min_w_t = time(game, 0 , w)
        min_w_p = 0
        for p in player_indices[1:]:
            if time(game, p , w) < min_w_t:
                min_w_t = time(game, p, w)
                min_w_p = p
        re[min_w_p].append(word_at(game,w))
    return re
