...
def is_always_roll(strategy, goal=GOAL):
    ...
    for s1 in range(goal):
        for s2 in range(goal):
            if strategy(s1,s2)!= last:
                return False
    return True
    # END PROBLEM 7
...
def make_averaged(original_function, total_samples=1000):
    ...
    def re_fun(*args):
        ...
        for i in range(total_samples):
            sum_sc += original_function(*args)
        return sum_sc / total_samples
    return re_fun
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    a_fun = make_averaged(roll_dice, total_samples)
    max_return , sum_max = 1 , 0
    for i in range(10):
        j = i+1
        sum_cur = a_fun(j,dice)
        if sum_cur > sum_max:
            sum_max = sum_cur
            max_return = j
    return max_return
...
def tail_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    #if tail_points(score) >= threshold and tail_points(opponent_score) >= threshold:
        return 0
    else:
        return num_rolls
    # END PROBLEM 10
