...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    ex = make_city('ex', lat, lon)
    if distance(ex, city_a) < distance(ex, city_b):
...
def sprout_leaves(t, leaves):
    ...
    return tree( label(t), [sprout_leaves(bt, leaves) for bt in branches(t)] )
...
def add_trees(t1, t2):
    ...
    newb = []
    lenb_t1, lenb_t2 = len(branches(t1)), len(branches(t2))
    for i in range(min(lenb_t1, lenb_t2)):
        newb += [add_trees(branches(t1)[i], branches(t2)[i])]
    if lenb_t1 < lenb_t2:
        for j in range(lenb_t1, lenb_t2):
            newb += [branches(t2)[j]]
    else:
        for j in range(lenb_t2, lenb_t1):
            newb += [branches(t1)[j]]
